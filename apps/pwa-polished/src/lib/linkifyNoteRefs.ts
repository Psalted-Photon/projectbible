...
/**
 * Every reference in a note, in order, for the chip layout cross-reference and
 * parallel-passage notes use. Returns [] when the note is prose with references
 * threaded through it rather than a list of them.
 */
export function noteRefList(
  body: string,
  contextBook: string,
  contextChapter: number,
): { label: string; ref: string; book: string }[] {
  const out: { label: string; ref: string; book: string }[] = [];
  let i = 0;
  ...
  while (i < body.length) {
    if (body[i] === REF_OPEN) {
      ...
      const close = body.indexOf(REF_CLOSE, sep === -1 ? i : sep);
      if (sep !== -1 && close !== -1) {
        const target = parseOsisRef(body.slice(i + 1, sep));
        const label = body.slice(sep + 1, close).replace(/[\x1A\x1B]/g, '');
        if (target) {
          out.push({ label, ref: `${target.book} ${target.chapter}:${target.verse}`, book: target.book });
        }
        i = close + 1;
        continue;
      }
    ...
    let j = i;
    while (j < body.length && !'\x17\x18\x19\x1A\x1B'.includes(body[j])) j++;
    for (const m of findRefs(body.slice(i, j), {
      requireBook: false,
      contextBook,
      contextChapter,
      strict: false,
    })) {
      out.push({ label: m.raw, ref: `${m.book} ${m.chapter}:${m.verse}`, book: m.book });
    }
    i = Math.max(j, i + 1);
  }
