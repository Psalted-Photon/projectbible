#!/usr/bin/env node
/**
 * extract-wj-spans-kjv.mjs
 *
 * Parses the KJV USFX XML (from eBible.org) and extracts character-level
 * <wj>...</wj> spans (Jesus' spoken words) for every NT verse that contains them.
 *
 * Output: data/processed/wj-spans-kjv.json
...
const USFX_PATH = join(repoRoot, 'data-sources', 'kjv-usfx', 'eng-kjv_usfx.xml');
const OUT_DIR = join(repoRoot, 'data', 'processed');
const OUT_PATH = join(OUT_DIR, 'wj-spans-kjv.json');

if (!existsSync(OUT_DIR)) mkdirSync(OUT_DIR, { recursive: true });

console.log('📖 Loading KJV USFX file...');
